...
> [!NOTE]
> **v0.1.4 지원 · 🔶 코드 확인**: 스타트레일 전용 프로파일(`startrail`)이 탑재되어, 키음 `02`(시작)와 `03`(끝) 기반의 롱노트 및 키음 `04`/`05` 게이트 개폐 페어링이 격자에 시각화됩니다.
> 실제 차트 15곡(MARENOIA 392노트 = 홀드 28 · 게이트 2 등, 짝 183개)에서 게임 모드 코드를 옮긴 오라클과 짝이 모두 일치했습니다. 게임을 돌려 본 플레이 확인은 아직입니다.
...
* 레인마다 **대기 칸이 하나뿐**입니다. `03` 이 오기 전에 같은 레인에 `02` 가 또 오면 앞의 `02` 는 **버려집니다**(`MissingEndNotes`). 에디터는 그 자리를 `ReplacedHead` 로 지목합니다.
* 게이트 키음 `04`/`05` 는 어느 채널에 찍든 게임 레인 9번 하나로 모여 짝지어집니다. 에디터도 채널을 가리지 않고 짝짓고, 채널이 다르면 두 노트를 선으로 잇습니다.
