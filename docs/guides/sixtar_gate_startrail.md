...
> [!NOTE]
> **v0.1.4 실측 검증 완료**: 스타트레일 전용 프로파일(`startrail`)이 탑재되어, 키음 `02`(시작)와 `03`(끝) 기반의 롱노트 및 게이트 개폐 페어링이 격자에 시각화됩니다 (실측 차트 MARENOIA 392노트 검증 완료).
> 차트 상단에 `#BMSEDITER_PROFILE startrail` 헤더를 넣거나 메인 화면 콤보박스에서 프로파일을 선택해 작업하십시오.
> 프로파일 사양 상세 → **[🎮 게임 프로파일 단일 표](../specifications/game_profiles.md#스타트레일-startrail--①-keyvalue)**
