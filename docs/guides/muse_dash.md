...
* **BPM 변경 작성법**:
  ...
  * BMS Editer에는 곡 도중 BPM 변경을 **입력하는 기능이 없습니다.** 텍스트 편집기로 `#BPMxx` + `#xxx08:` 줄을 넣으면 에디터가 읽어 격자에 반영하고, 저장할 때 원문 그대로 되돌려 씁니다. 채널 `03` 으로 적힌 BPM 변화도 에디터는 읽지만 이 게임은 무시하므로 `08` 로 바꿔 두십시오.
