...
### 4. 이슈 및 품질 관리 (`issues/`)
* **[known_issues.md](issues/known_issues.md)**: 핵심 과제 해결(조건 블록, 키음 믹싱, 렌더링, 🎛️ 컨트롤 패널, 게임 프로파일 및 홀드 자동 페어링)과 306개 단위 테스트 현황, 남은 과제(Undo/Redo, 키음 지연, 비디오 오버플로우 등), 점검에서 찾은 확인 필요 항목을 총망라합니다. **실사용에서 터지는 자리**(자동 저장 없음, 전역 예외 처리기 없음, 파일을 열어도 재생이 안 멈춤, 없는 키음에 표시 없음 등)가 맨 앞에 있고, **재생할 때 걸리는 것**(편집 모드에서 멈추는 클릭이 노트를 찍거나 지움, Space 가 엉뚱한 버튼을 누름, 고친 키음 파일이 반영 안 됨, 키음 지연이 구간마다 다름 등)이 뒤따릅니다.
