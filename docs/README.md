...
### 4. 이슈 및 품질 관리 (`issues/`)
* **[known_issues.md](issues/known_issues.md)**: 핵심 과제 해결(조건 블록, 키음 믹싱, 렌더링, 🎛️ 컨트롤 패널, 게임 프로파일 및 홀드 자동 페어링)과 306개 단위 테스트 현황, 남은 과제(Undo/Redo, 키음 지연, 비디오 오버플로우 등), 2026-09-22 점검에서 찾은 확인 필요 항목(조건 블록 저장 순서, 오프셋 스크럽 등)을 총망라합니다.
