...
### 4. 이슈 및 품질 관리 (`issues/`)
* **[known_issues.md](issues/known_issues.md)**: 핵심 과제 해결(조건 블록, 키음 믹싱, 렌더 무결성, 🎛️ 컨트롤 패널, 게임 프로파일 및 홀드 자동 페어링)과 306개 단위 테스트 현황, 남은 과제(Undo/Redo, 비디오 오버플로우 등)를 총망라합니다.
* **[authoring_time.md](issues/authoring_time.md)**: ⏱️ **채보 작성 시간 분석과 개선 성과.** 초기 실측 기록(뮤즈 대시 340노트 = 약 4시간)과 병목 원인 분석, 그리고 이를 해소하기 위해 v0.1.4에 구축된 프로파일 기반 홀드 자동 페어링/몸통 렌더링의 개선 내역과 향후 과제를 다룹니다.
