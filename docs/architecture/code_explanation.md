...
| 파일명 | 역할 및 핵심 기술 |
| :--- | :--- |
| **[BmsParser.cs](../../bms%20editer/Services/BmsParser.cs)** | 파싱 파이프라인 Core입니다. `BmsParseResult` 정의와 2패스 `Parse()` — 1패스에서 헤더·`#WAV` 정의를 모으고, 2패스에서 데이터 줄을 노트로 풀며 편집 대상이 아닌 줄은 원문 그대로 보존(`PreservedLines`)합니다. 조건 블록(`#RANDOM`/`#IF`/`#SWITCH`)의 갈래(`BranchId`)도 여기서 추적합니다. 아래 4개 파티션으로 나뉘어 있습니다. `Parse(filePath, documentPath)` 는 내용을 filePath 에서 읽되 상대 경로는 documentPath 의 폴더 기준으로 찾습니다(자동 저장본을 원래 자리의 문서로 되살릴 때 씀). |
...
| **[DocumentSnapshotStore.cs](../../bms%20editer/Services/DocumentSnapshotStore.cs)** | 🛟 자동 저장본·저장 이력·비정상 종료 감지입니다. 문서마다 `recovery/snapshots/<곡폴더>_<파일명>_<해시>/` 폴더에 자동 저장본(`_auto`)과 덮어쓰기 직전 사본(`_before-save`)을 종류별 최근 10벌씩 남기고, 한 달 넘게 안 쓴 문서 폴더는 지웁니다. 실행마다 `recovery/sessions/` 에 세션 파일을 두어 "저장 안 된 작업이 있는가, 최근 자동 저장본은 무엇인가"를 적고, 정상 종료하면 지웁니다. 주인 프로세스(번호+시작 시각)가 없는 세션 파일이 남아 있으면 그 실행은 죽은 것이라 복구 대상입니다. 복구를 기다리는 스냅숏은 개수가 넘쳐도 지우지 않습니다. 스냅숏의 `#WAV` 경로는 원래 곡 폴더 기준입니다. |
| **[ErrorLog.cs](../../bms%20editer/Services/ErrorLog.cs)** | 예상하지 못한 예외를 `logs/error-*.log` 로 한 건씩 남깁니다(최근 30건). 같은 예외가 5초 안에 되풀이되면 한 번만 적고, 쓰기에 실패해도 절대 던지지 않습니다. |
| **[AppDataPaths.cs](../../bms%20editer/Services/AppDataPaths.cs)** | 앱이 만드는 파일의 자리(`%LocalAppData%` 의 `bms editer` 폴더 아래 `logs` · `recovery` · `WebView2`)를 한 곳에서 정합니다. |
...
| **[MainWindowViewModel.FileIO.cs](../../bms%20editer/ViewModels/MainWindowViewModel.FileIO.cs)** | 입출력 파티션입니다. BMS 열기/저장, 원본 인코딩 보존과 손실 시 UTF-8 폴백, OGG 비동기 디코딩(`LoadOggAsync`), 비디오 연결, 새 문서 초기화를 담당합니다. 열기는 `LoadDocument(내용 경로, 문서 경로)` 한 곳을 지나고, 저장은 덮어쓰기 직전 파일을 저장 이력으로 넘긴 뒤 씁니다. |
| **[MainWindowViewModel.Safety.cs](../../bms%20editer/ViewModels/MainWindowViewModel.Safety.cs)** | 🛟 안전장치 파티션입니다. 저장 안 된 변경이 있을 때만 자동 저장(`AutoSave`, 바뀐 게 없으면 다시 쓰지 않음)·비상 저장(`EmergencySave`), 자동 저장본을 원래 경로·인코딩의 "저장 안 된 문서"로 되살리기(`RecoverFromSnapshot`), 저장 이력 남기기, 문서가 깨끗해지면 세션에 "복구할 것 없음" 적기를 맡습니다. `Snapshots` 가 비어 있으면(테스트·디자이너) 디스크를 건드리지 않고, 여기서는 아무것도 던지지 않습니다. |
...
| **[NoteSearchViewModel.cs](../../bms%20editer/ViewModels/NoteSearchViewModel.cs)** | 🧰 검색/삭제/교체 창 뷰모델입니다. 선택 여부·노트 종류(롱 = 홀드 짝)·마디 범위·키음 범위·레인 조건으로 노트를 찾아 선택·삭제·번호 변경·마디 옮겨 복사를 실행합니다. [삭제]·[번호 일괄 변경]은 대상이 20개(`ConfirmThreshold`) 이상이면 먼저 묻고, 확인 창을 띄울 수 없으면 하지 않습니다. |
...
| **[MainWindow.Safety.cs](../../bms%20editer/MainWindow.Safety.cs)** | 🛟 안전장치 파티션입니다. `EnableSafety` 로 켜지면 세션을 시작하고 2분 자동 저장 타이머를 돌리며, 창을 열 때 지난 실행이 남긴 작업의 복구를 묻고(복구 / 복구 안 함 / 취소), 정상 종료 때 세션을 지웁니다. `App` 이 넘겨준 처리되지 않은 예외는 재생 정지 → 비상 저장 → "오류" 창([계속 작업]/[끄기])으로 받습니다. 파일 메뉴의 [자동 저장·오류 기록 폴더 열기]도 여기 있습니다. 테스트가 띄우는 창은 켜지 않습니다. |
...
| **[ConfirmWindow.axaml / .cs](../../bms%20editer/Views/ConfirmWindow.axaml.cs)** | 공용 확인 대화상자입니다. 확인/취소 2지선다, 알림 전용, 저장/저장 안 함/취소 3지선다를 제공합니다. 창을 그냥 닫으면 `Cancel` 이 되도록 기본값을 0 으로 둡니다. 취소 없이 글자를 정한 두 버튼으로 묻는 `ShowChoiceAsync`(오류 창의 계속/끄기)도 있습니다. |
...
| 파일 | 역할 |
| :--- | :--- |
| **[Program.cs](../../bms%20editer/Program.cs) · [App.axaml.cs](../../bms%20editer/App.axaml.cs)** | 진입점입니다. `App` 이 메인 창을 만들며 안전장치를 켜고(`EnableSafety`), UI 스레드의 처리되지 않은 예외(`Dispatcher.UIThread.UnhandledException`)를 받아 메인 창에 넘깁니다. `Program` 은 백그라운드 스레드·버려진 Task 의 예외와 앱 시작 실패를 `ErrorLog` 로 남깁니다(예전의 exe 옆 `crash.log` 는 없앴습니다). |
