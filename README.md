...
3. **고속 오디오 분석 및 파형 가시화**
   - 초당 400칸(한 칸 2.5ms) 해상도로 전체 배경 오디오 파형 출력. 곡 길이와 무관하게 해상도가 유지됩니다.
   - **피크 + RMS 2중 표시**: 옅게 깔린 피크 포락선 위로 RMS 몸통을 겹쳐 그려, 지속음 위로 어택만 뿔처럼 솟아 보입니다.
   - **구간 최댓값 집계**: 화면 블록 하나가 덮는 버킷을 전부 훑어 최댓값을 씁니다. 줌을 줄여도 드럼 한 방이 화면에서 사라지지 않고, 줌을 바꿔도 파형 모양이 달라지지 않습니다.
   - **온셋(Onset) 검출**: 감쇄만 늦춘 어택 포락선에서 적응 임계로 봉우리를 골라내고, 에너지가 오르기 시작한 지점까지 되짚어 마커를 세웁니다. 합성 클릭 트랙 기준 실측 오차 2ms 안쪽 (박자 및 마디 씽크 조율 편리).
   - 표시 자체(파형 두께·색·마커 밀도)는 v0.1.0 과 같습니다. 파형은 격자를 맞추기 위한 배경이라 그 위에 겹치는 마디선과 마커를 덮지 않아야 합니다.

3-1. **음원 오프셋 (Audio Offset)**
   - 곡의 첫 박이 파일 맨 앞에서 마디 경계에 정확히 떨어지는 일은 드뭅니다. 이 값(ms)만큼 **파형·온셋·재생 커서를 뒤로 밀어** 격자에 맞춥니다. 격자와 노트는 움직이지 않습니다.
   - **자동 버튼**: 검출된 어택이 16분 격자에 가장 잘 붙는 값을 찾습니다. 후보를 16분음표의 ±절반으로만 훑기 때문에, 온셋이 격자에 대해 주기적이어서 생기는 "한 칸 밀린 답"이 뽑히지 않습니다.
   - `#BMSEDITER_OFFSET` 헤더로 저장됩니다. BMS 표준이 아니라 다른 플레이어는 무시하며, 값이 0이면 아예 쓰지 않아 파일이 그대로 남습니다.
   - 없을 때 실제로 겪은 일: BPM 141 이 맞는데도 마디선과 드럼 어택이 22.9ms(세로 줌 8에서 6px) 어긋나 보여서, 맞는 BPM 을 틀렸다고 판단하게 됩니다.
