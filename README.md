...
10. **BMS 저장 기능**
   ...
   - **저장 이력**: `.bak` 은 한 벌뿐이라, 덮어쓰기 직전의 파일을 따로 최근 10벌까지 남깁니다(아래 15번).
   - **조건 블록(`#RANDOM`/`#IF`/`#SWITCH`) 분기 보존**: 분기 식별자(`BranchId`)와 원본 줄 순서(`Order`)를 추적하여 갈래별 노트를 나눠 저장합니다.
     단, 저장할 때 조건 줄도 **마디 순으로 다시 정렬**하기 때문에 줄 배치에 따라 순서가 섞일 수 있고, 블록 안의 헤더·주석은 헤더 영역으로 옮겨집니다.
     조건 블록이 든 차트는 저장 결과를 한 번 확인하십시오 → [known_issues](docs/issues/known_issues.md#-2026-09-22-점검에서-찾은-것-확인-필요)
...
11. **노트 검색/삭제/교체**
   ...
   - 삭제·번호 변경은 대상이 20개 이상이면 개수를 보여 주고 먼저 묻습니다(되돌리기가 아직 없습니다).
...
15. **🛟 안전장치: 자동 저장 · 비정상 종료 복구 · 저장 이력 · 오류 기록**
   - 저장 안 된 변경이 있으면 **2분마다 자동 저장**합니다. 앱이 죽으면 다음에 켤 때 **"작업 복구"** 를 물어, 마지막 자동 저장본을 원래 파일 자리의 "저장 안 됨(`*`)" 문서로 되살립니다. 원래 파일은 [저장]을 눌러야 바뀝니다.
   - 예상하지 못한 오류가 나도 앱이 꺼지지 않습니다. 그 자리에서 비상 저장한 뒤 [계속 작업] / [끄기] 를 묻습니다.
   - [저장]할 때마다 덮어쓰기 직전의 파일을 **저장 이력**으로 남깁니다(문서마다 최근 10벌).
   - 파일은 모두 `%LocalAppData%` 의 `bms editer` 폴더(`recovery` · `logs`)에 있고, **파일 메뉴 → [자동 저장·오류 기록 폴더 열기]** 로 바로 열립니다. 스냅숏은 원래 곡 폴더 기준 경로로 적혀 있어, 곡 폴더에 복사해 넣으면 그대로 열립니다.
   - 한계와 자세한 동작: [known_issues G. 안전장치](docs/issues/known_issues.md#-g-안전장치-해결-완료-)
...
- **`Services`**:
  ...
  - `DocumentSnapshotStore` · `ErrorLog`: 자동 저장본·저장 이력·비정상 종료 감지와 오류 기록 (`%LocalAppData%` 아래).
...
| **품질/이슈** | [known_issues.md](docs/issues/known_issues.md) | **남은 일과 이미 고친 것.** 🧨 실사용에서 터지는 자리(재생 정지·키음 표시 등, 자동 저장·크래시 대비는 해결), 🎧 재생할 때 걸리는 것, Undo/Redo, 키음 지연, 커밋별 해결 기록이 여기 있습니다. |
