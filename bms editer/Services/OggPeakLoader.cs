...
public static class OggPeakLoader
{
    ...
    // 이 세기 아래는 마커로 세우지 않는다. v0.1.0 과 같은 마커 밀도를 내는 값이다.
    //
    // 참고로 실제 음원(BPM 141, 171초)에서 세기별로 16분 격자에 붙는 비율은 이렇다.
    //   0.00~0.20  13%    0.20~0.35  17%
    //   0.35~0.50  31%    0.50~0.75  55%    0.75~1.00  86%
    // 약한 봉우리는 리버브 꼬리나 지속음의 출렁임이라 격자와는 상관이 적다.
    // 격자를 맞추는 기준으로는 진하게 그려지는 0.45 이상만 보면 된다.
    // 이 값을 올리면 화면은 깨끗해지지만 v0.1.0 과 다른 그림이 된다.
