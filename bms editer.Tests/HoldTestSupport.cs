...
internal static class HoldTestSupport
{
    ...
    // 에디터 뷰모델이 짝 맞추기에 넘기는 것과 같은 표. (MainWindowViewModel.BuildWavTexts)
    public static IReadOnlyDictionary<string, string> WavTexts(BmsParseResult parsed)
    {
        var texts = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        foreach (var item in parsed.WavItems)
            texts[item.Key] = string.IsNullOrEmpty(item.SourceText) ? item.FileName : item.SourceText;

        return texts;
    }

    // 사람이 눈으로 확인할 그림과 요약을 남기는 곳. 테스트가 끝나도 지우지 않는다.
    ...
    public static bool LooksLikeAnyHoldBody(Color pixel, IEnumerable<string> kinds) =>
        kinds.Any(kind => LooksLikeHoldBody(pixel, kind));

    public static string SafeFileName(string text) =>
        new(text.Select(c => Path.GetInvalidFileNameChars().Contains(c) ? '_' : c).ToArray());
}
